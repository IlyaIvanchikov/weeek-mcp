...
14. **Task shape omits `assignee` and `board`** — `WeeekTask` returns only `{id,title,description,projectId,boardColumnId,completed}`, so `get_task`/`create_task` can't confirm who a task is assigned to or which board it's on. Surfacing `assignee`/`boardId` would let callers verify assignment (found while validating assignee-by-name live). Consider a `list_boards`/`list_columns` read tool too, so `move_task` targets are discoverable.
